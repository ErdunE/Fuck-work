...
class AnswerRequest(BaseModel):
    ...
    question: str = Field(
        ..., min_length=5, max_length=1000, description="Question to answer"
    )
    context_type: Literal["job_application", "recruiter", "general"] = Field(
        "job_application", description="Type of context for answer generation"
    )
...
class AnswerResponse(BaseModel):
    ...
    sources_used: List[str] = Field(
        ..., description="Which user data sources were used"
    )
