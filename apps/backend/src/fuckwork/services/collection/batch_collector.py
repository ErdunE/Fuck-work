...
class BatchCollector:
    ...
    def run_batch(self, search_configs: List[Dict], max_retries: int = 2) -> Dict:
        """
            ...
            while attempt < max_retries and not success:
                try:
                    ...
                    if df is not None and not df.empty:
                        ...
                        logger.info(
                            f"  ✓ Collected {len(jobs)}, Saved {save_stats['saved']}, Duplicates {save_stats['duplicates']}"
                        )
                        success = True
                    else:
    ...
    def _print_summary(self, stats: Dict):
        ...
        for category, cat_stats in stats["by_category"].items():
            logger.info(
                f"  {category:20s}: {cat_stats['queries']:3d} queries, "
