...
def print_statistical_analysis(results: list):
    ...
    print(f"   Jobs with 0 positive: {pos_counts.count(0):4d} ({pos_counts.count(0)*100/len(results):5.1f}%)")
    print(f"   Jobs with 0 negative: {neg_counts.count(0):4d} ({neg_counts.count(0)*100/len(results):5.1f}%)")
...
def print_sample_jobs(results: list):
    ...
    for range_name, filter_fn in ranges:
        ...
        for r in random.sample(filtered, min(2, len(filtered))):
            ...
            print(f"      Score: {r['score']:.1f} | Level: {r['level']} | +{r['positive_count']}/-{r['negative_count']}")
            if r['red_flags']:
