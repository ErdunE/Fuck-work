...
def calculate_quality_signals(job: dict) -> dict:
    ...
    signals = {
        ...
        "has_salary_range": bool(
            re.search(r"\$\s*\d{2,3}[,.]?\d{0,3}\s*[-–to]+\s*\$?\s*\d{2,3}", jd)
        ),
        ...
        "has_responsibilities": bool(
            re.search(r"\b(responsibilities|what you.ll do|your role|in this role)\b", jd)
        ),
        "has_requirements": bool(
            re.search(r"\b(requirements|qualifications|what we.re looking|who you are)\b", jd)
        ),
        "has_about_us": bool(
            re.search(r"\b(about us|about the company|who we are|our mission)\b", jd)
        ),
        "has_interview_process": bool(
            re.search(r"\b(interview process|hiring process|application process)\b", jd)
        ),
        # Team info
        "has_team_info": bool(
            re.search(r"\b(our team|the team|your team|engineering team|product team)\b", jd)
        ),
        "has_manager_info": bool(re.search(r"\b(report(s|ing)? to|manager|director)\b", jd)),
        # Specificity
        "has_tech_stack": bool(
            re.search(r"\b(python|java|javascript|react|aws|sql|docker|kubernetes)\b", jd)
        ),
        "has_years_exp": bool(re.search(r"\d+\+?\s*years?\s*(of\s*)?(experience|exp)", jd)),
        # === NEGATIVE SIGNALS ===
        # Recruiter/Agency
        "is_staffing_company": any(
            x in company
            for x in [
                "apex systems",
                "kforce",
        ...
        # Vague
        "has_competitive_salary": bool(
            re.search(r"\bcompetitive\s*(salary|compensation|pay)\b", jd)
        ),
...
def analyze_quality_distribution(jobs: list):
    ...
    for tier_name, tier_jobs in tiers.items():
        ...
        for s in samples:
            ...
            if pos_triggers:
                print(
                    f"      ✅ {', '.join(pos_triggers[:4])}{'...' if len(pos_triggers) > 4 else ''}"
                )
            if neg_triggers:
