...
def find_suspicious_jobs(sql_content: str, limit: int = 20) -> list:
    ...
    staffing_companies = ['apex systems', 'kforce', 'insight global', 'robert half', 'teksystems', 'randstad']
    ...
    for match in matches:
        try:
            ...
            # Categorize
            if any(kw in jd_text for kw in recruiter_keywords) and len(categories['recruiter_language']) < limit:
                categories['recruiter_language'].append(job)
            elif any(sc in company for sc in staffing_companies) and len(categories['staffing_company']) < limit:
                categories['staffing_company'].append(job)
            elif any(kw in title for kw in contract_keywords) and len(categories['contract_title']) < limit:
...
def test_edge_cases():
    ...
    for category, jobs in categories.items():
        ...
        for job in jobs[:5]:  # Test first 5 of each category
            ...
            print(f"   Score: {result['authenticity_score']:.1f} | Level: {result['level']} | Confidence: {result['confidence']}")
