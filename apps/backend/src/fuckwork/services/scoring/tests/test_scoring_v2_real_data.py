...
def test_with_real_data():
    ...
    for r in sample_results:
        ...
        print(f"   Platform: {r['platform']} | Score: {r['score']:.1f} | Level: {r['level']} | Confidence: {r['confidence']}")
        if r['red_flags']:
            print(f"   ❌ Red flags ({r['negative_count']}): {r['red_flags'][:2]}{'...' if len(r['red_flags']) > 2 else ''}")
        if r['positive_signals']:
            print(f"   ✅ Positive ({r['positive_count']}): {r['positive_signals'][:2]}{'...' if len(r['positive_signals']) > 2 else ''}")
    ...
    print(f"\n📊 Signal Counts:")
    print(f"   Avg positive signals per job: {sum(positive_signal_counts)/len(positive_signal_counts):.1f}")
    print(f"   Avg negative signals per job: {sum(negative_signal_counts)/len(negative_signal_counts):.1f}")
    print(f"   Jobs with 0 positive signals: {positive_signal_counts.count(0)} ({positive_signal_counts.count(0)*100/len(positive_signal_counts):.1f}%)")
    print(f"   Jobs with 0 negative signals: {negative_signal_counts.count(0)} ({negative_signal_counts.count(0)*100/len(negative_signal_counts):.1f}%)")
    ...
    high_score_short_jd = [r for r in results if r['score'] >= 90 and len(r['positive_signals']) <= 2]
    if high_score_short_jd:
        print(f"\n🔍 High score (>=90) but few positive signals ({len(high_score_short_jd)} jobs):")
        for r in high_score_short_jd[:3]:
            print(f"   - {r['title'][:40]} @ {r['company'][:20]} (score={r['score']:.1f}, +{r['positive_count']}/-{r['negative_count']})")
