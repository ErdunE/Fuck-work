                        ...
                        results[idx].update(
                            {
                                ...
                                "industry": llm_result["industry"],
                                "industry_display": get_industry_display_name(
                                    llm_result["industry"]
                                ),
                                "specialties": llm_result["specialties"],
                                "specialty_displays": get_specialty_display_names(
                                    llm_result["specialties"]
                                ),
                                "classification_method": "llm",
                                "confidence": llm_result.get("confidence", "medium"),
                            }
...
def add_classification_to_job(
    job: Dict[str, Any], classifier: JobClassifier = None
) -> Dict[str, Any]:
    """
...
def classify_unclassified_jobs(
    db_session, batch_size: int = 100, use_llm: bool = True
) -> Dict[str, int]:
    """
    ...
    stats = {"total": total, "classified": 0, "rules": 0, "llm": 0, "errors": 0}
