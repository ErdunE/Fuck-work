...
class JobEnricher:
    ...
    def _derive_job_level(self, title: str) -> str:
        ...
        if any(
            ...
            for keyword in ["new grad", "entry level", "graduate", "entry-level"]
        ):
            return "new_grad"
    ...
    def _derive_work_mode(self, job_dict: Dict) -> str:
        ...
        if any(
            keyword in jd_text
            for keyword in ["fully remote", "100% remote", "work from home", "wfh"]
        ):
            return "remote"
    ...
    def _derive_salary(self, platform_metadata: Dict) -> Dict[str, Optional[float]]:
        ...
        """
        result = {"min": None, "max": None, "interval": None}
