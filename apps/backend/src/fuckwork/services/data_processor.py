...
AGGREGATORS = [
    'Lensa', 'Jobgether', 'ZipRecruiter', 'Dice', 'Hired',
    'Ladders', 'Wellfound', 'AngelList'
]

STAFFING_FIRMS = [
    'Robert Half', 'Jobot', 'Insight Global', 'Apex Systems',
    'TEKsystems', 'Kforce', 'Randstad', 'Adecco', 'CyberCoders',
    'Kelly Services', 'ManpowerGroup', 'Hays', 'Aerotek',
    'Phyton Talent', 'Crawford Thomas', 'Modis', 'Experis',
    'Collabera', 'Synergis', 'Motion Recruitment'
]
...
class JobDataProcessor:
    ...
    def _mark_duplicates(self):
        ...
        jobs = self.session.query(
            Job.id, Job.job_id, Job.title, Job.company_name, 
            Job.location, Job.platform, Job.created_at
        ).all()
        ...
        logger.info(f"  ✓ Marked {duplicates_count} duplicates (cross-platform: {cross_platform}, same-platform: {same_platform})")
    ...
    def _mark_nan_companies(self):
        ...
        nan_jobs = self.session.query(Job).filter(
            or_(
    ...
    def _mark_low_quality_sources(self):
        ...
        logger.info(f"  ✓ Marked {aggregator_count} aggregator jobs, {staffing_count} staffing firm jobs")
    ...
    def _extract_skills_batch(self, batch_size: int):
        ...
        while True:
            ...
            if offset % 5000 == 0:
                self.session.commit()
                logger.info(f"    Processed {offset} jobs, extracted skills for {extracted_count}...")
    ...
    def _extract_experience_batch(self, batch_size: int):
        ...
        while True:
            ...
            if offset % 5000 == 0:
                self.session.commit()
                logger.info(f"    Processed {offset} jobs, extracted experience for {extracted_count}...")
