...
class JobService:
    ...
    def search_jobs(
        ...
        if filters:
            ...
            # Country filter
            if filters.country:
                conditions.append(text("derived_signals->'geo'->>'country' = :country"))
            ...
            if filters.has_red_flags is not None:
                if filters.has_red_flags is False:
                    # No red flags
                    conditions.append(
                        or_(
                            Job.red_flags is None,
                            func.jsonb_array_length(Job.red_flags) == 0,
                        )
                    )
                else:
                    # Has red flags
                    conditions.append(func.jsonb_array_length(Job.red_flags) > 0)
            ...
            if filters.exclude_companies:
                ...
                conditions.append(
                    ~func.lower(Job.company_name).in_(
                        [c.lower() for c in filters.exclude_companies]
                    )
                )
            ...
            if filters.include_companies_only:
                ...
                conditions.append(
                    func.lower(Job.company_name).in_(
                        [c.lower() for c in filters.include_companies_only]
                    )
                )
            ...
            if filters.keywords_in_description:
                ...
                for keyword in filters.keywords_in_description:
                    conditions.append(Job.jd_text.ilike(f"%{keyword}%"))
