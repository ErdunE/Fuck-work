...
def batch_events(
    ...
    run = (
        db.query(ApplyRun)
        .filter(ApplyRun.id == request.run_id, ApplyRun.user_id == current_user.id)
        .first()
    )
...
def get_run_detail(
    ...
    run = (
        db.query(ApplyRun)
        .filter(ApplyRun.id == run_id, ApplyRun.user_id == current_user.id)
        .first()
    )
    ...
    first_event = (
        ...
        .filter(ObservabilityEvent.run_id == run_id)
    ...
    last_event = (
        db.query(ObservabilityEvent)
        .filter(ObservabilityEvent.run_id == run_id)
        .order_by(ObservabilityEvent.ts.desc())
        .first()
    )
