...
from src.fuckwork.api.auth import (
    create_access_token,
    verify_password,
    hash_password,
    get_current_user,
...
def register(request: RegisterRequest, db: Session = Depends(get_db)):
    ...
    user = User(email=request.email, password_hash=password_hash, is_active=True)
    db.add(user)
...
def login(request: LoginRequest, response: Response, db: Session = Depends(get_db)):
    ...
    expires_at = (
        datetime.utcnow() + timedelta(minutes=JWT_EXPIRATION_MINUTES)
    ).isoformat()
    ...
    print(
        f"[Auth] Login successful for user {user.id}, token_version={user.token_version}, cookie set"
    )
...
def logout(
    ...
    print(
        f"[Auth] Logout for user {current_user.id}, token_version incremented to {current_user.token_version}, cookie cleared"
    )
...
def get_extension_token(current_user: User = Depends(get_current_user)):
    ...
    **Flow**:
    ...
    return ExtensionTokenResponse(
        token=token, expires_in=900  # 15 minutes = 900 seconds
    )
