...
def get_apply_tasks(
    status: Optional[str] = Query(
        None,
        description="Filter by status: queued, in_progress, needs_user, success, failed, canceled",
    ),
    ...
    """
    # Base query
    query = db.query(ApplyTask).filter(ApplyTask.user_id == current_user.id)
...
def get_apply_task(
    ...
    task = (
        db.query(ApplyTask)
        .filter(ApplyTask.id == task_id, ApplyTask.user_id == current_user.id)
        .first()
    )
