...
def compute_highest_degree(education_records: List[UserEducation]) -> Optional[str]:
    ...
    degree_hierarchy = {"PhD": 4, "MS": 3, "BS": 2, "AS": 1}
    ...
    for edu in education_records:
        ...
        if "phd" in degree_lower or "doctorate" in degree_lower or "ph.d" in degree_lower:
            ...
            or "ma" in degree_lower
            or "mba" in degree_lower
            or "m.s" in degree_lower
            or "m.a" in degree_lower
        ):
            ...
            or "ba" in degree_lower
            or "b.s" in degree_lower
            or "b.a" in degree_lower
        ):
...
def compute_years_of_experience(
    experience_records: List[UserExperience],
) -> Optional[int]:
    """
...
def compute_work_authorization_primitives(
    ...
    if any(
        ...
        for keyword in ["green card", "permanent resident", "pr", "greencard"]
    ):
...
def get_derived_profile(
    ...
    if years_of_experience is not None:
        source_fields["years_of_experience"] = [
            "user_experience.start_date",
            "user_experience.end_date",
    ...
    derived = DerivedProfile(
        ...
        willing_to_relocate=(
            ...
            if (profile and profile.willing_to_relocate is not None)
            else False
        ),
        government_employment_flag=(
            profile.government_employment_history
            if (profile and profile.government_employment_history is not None)
            else False
        ),
        # Skills
