...
def set_active_session(
    ...
    task = (
        db.query(ApplyTask)
        .filter(ApplyTask.id == request.task_id, ApplyTask.user_id == current_user.id)
        .first()
    )
    ...
    print(
        f"[Active Session] Set for user {current_user.id}: run_id={request.run_id}, task_id={request.task_id}"
    )
...
def get_active_session(
    ...
    print(
        f"[Active Session] Retrieved for user {current_user.id}: run_id={session.run_id}, task_id={session.task_id}"
    )
