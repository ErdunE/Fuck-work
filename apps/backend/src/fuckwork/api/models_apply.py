...
class ApplyQueueRequest(BaseModel):
    ...
    job_ids: List[str] = Field(..., min_length=1, max_length=100, description="List of job IDs to queue")
    priority_strategy: Literal["decision_then_newest", "newest", "highest_score"] = Field(
        "decision_then_newest",
...
class ApplyTransitionRequest(BaseModel):
    ...
    to_status: Literal["queued", "in_progress", "needs_user", "success", "failed", "canceled"] = Field(
        ...,
